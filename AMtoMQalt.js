...
{input:"RR",  tag:"mo", output:"\u211D", tex:"Re", ttype:CONST, notexcopy:true},
...
function AMQTgetTeXsymbol(symb) {
	...
	if (symb.tex==null) {
		return (pre+((pre==''||symb.ttype==CONST)?symb.input:symb.input.toLowerCase()));
	} else {
		return (pre+symb.tex);
	}
}
...
return function(str) {
 AMQnestingDepth = 0;
  str = str.replace(/(&nbsp;|\u00a0|&#160;)/g,"");
  //only need this for question answer stuff
  ...
  //out = out.replace(/_\{(\d)\}/g,'_$1').replace(/\^\{(\d)\}/g,'^$1');
  console.log("gen tex: "+out);
  return out;
}
}();
...
function MQtoAM(tex) {
	...
	while ((i=tex.indexOf('\\left|'))!=-1) { //found a left |
		nested = 0; curi = i;
		do {
			lb = tex.indexOf('\\left|',curi+1);
			rb = tex.indexOf('\\right|',curi+1);
	...
	tex = tex.replace(/\\rightarrow/g,'rarr');
	tex = tex.replace(/\\left/g,'');
	tex = tex.replace(/\\right/g,'');
	//for question answers, replace with < and >
	tex = tex.replace(/\\langle/g,'<<');
	tex = tex.replace(/\\rangle/g,'>>');
	tex = tex.replace(/\\cdot/g,'*');
	tex = tex.replace(/\\infty/g,'oo');
	tex = tex.replace(/\\nthroot/g,'root');
	//for question answers, replace with DNE
	tex = tex.replace(/\\varnothing/g,'emptyset');
	tex = tex.replace(/\\{/g,'lbrace');
	tex = tex.replace(/\\}/g,'rbrace');
	tex = tex.replace(/_\{\s*\}/g,'').replace(/\^\{\s*\}/g,' ');
	tex = tex.replace(/\\/g,'');
	...
	tex = tex.replace(/{/g,'(');
	tex = tex.replace(/}/g,')');
	tex = tex.replace(/lbrace/g, '{');
	tex = tex.replace(/rbrace/g, '}');
